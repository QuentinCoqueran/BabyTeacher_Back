...
export class ContractController {
    ...
    async getById(req: Request, res: Response) {
        ...
        try {
            if (req.query.id) {
                contract = await ContractService.getInstance().getById(parseInt(<string>req.query.id));
            }
            if (contract) {
                res.send({
                    response: contract
    ...
    async deleteContract(req: Request, res: Response) {
        try {
            const contract = await ContractService.getInstance().delete(parseInt(<string>req.body.id));
            if (contract) {
                res.send({
                    response: contract
                });
